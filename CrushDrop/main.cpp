...
void swap(piece p1, piece p2) {
	...
	grid[p1.row][p2.col] = p1;
	grid[p2.row][p2.col] = p2;
}

int main() {
	srand(time(0));
	RenderWindow window(VideoMode(740,480),"MatchCrush");
	...
	while (window.isOpen()) {
		...
		if (click ==2) {
			x = pos.x / ts + 1;
			y = pos.y / ts + 1;
			if (abs(x - x0) + abs(y - y0) == 1) { swap(grid[y0][x0], grid[y][x]); isSwap = 1; click = 0; }
			else click = 1;
		}
		...
		if (isSwap && !isMoving) {
			swap(grid[y0][x0], grid[y][x]); isSwap = 0;
		}
